 ...
 * @param options Optional params. shouldExit - If true, this function will throw on error, target - Additional log
 * @example
...
const tryCatch = async (
  func: () => Promise<void>,
  errCallback: unknown[],
  options?: { shouldExit?: boolean; target?: string },
): Promise<void> => {
  try {
    await func();
  } catch (errRes) {
    ...
    errors.forEach((err) => {
      ...
      Log.error(options?.target ?? 'TryCatch', err);
    });
    if (options?.shouldExit) throw errRes;
  }
};
